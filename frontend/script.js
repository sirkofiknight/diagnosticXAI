...
let lastExplain   = null;   // cached /explain response (LR)
let abortCtrl     = null;   // AbortController for in-flight requests
...
async function refresh() {
    ...
    try {
        if (selectedModel === 'Logistic Regression') {
            ...
            // Full XAI (may be slow for SVM/KNN)
            ...
            // Contributions: use SHAP if available, else zeros
            const fnames = ['Academic Effort', 'Institutional Commitment', 'Wellness & Balance'];
            const keys   = ['studytime', 'attendance', 'wellbeing'];
            ...
            if (e.shap_values) {
                ...
                updateContribChart({ studytime: 0, attendance: 0, wellbeing: 0 }, `${selectedModel} — SHAP unavailable`);
            }
            ...
            dom.detailedText.innerHTML = `<div class='font-mono text-[11px] text-zinc-400 leading-relaxed'>
                ...
                ${cf ? `Counterfactual flip at ${cf.feature} = ${cf.flip_value}` : ''}
            </div>`;
        }
    } catch (err) {
...
async function loadMetricsTable() {
    ...
    try {
        ...
        dom.metricsTable.innerHTML = '<div class="text-red-400 text-xs p-3">Failed to load metrics. Is the server running? Has train_all.py been run?</div>';
    }
}
...
function selectModelFromTable(name) {
    ...
    refresh();
    loadMetricsTable(); // refresh active highlight
}

// ── XAI SHAP+LIME in comparison modal ────────────────────────────────────────
...
async function loadXaiForCurrentModel() {
    ...
    try {
        ...
        const shapVals = fnames.map(f => shapData.shap_values ? (shapData.shap_values[f] ?? 0) : 0);
        const limeVals = fnames.map(f => limeData.lime_weights ? (limeData.lime_weights[f] ?? 0) : 0);

        renderXaiShapChart(labels, shapVals);
        renderXaiLimeChart(labels, limeVals);
        renderXaiGroupedChart(labels, shapVals, limeVals, null);
...
function _makeBarChart(canvasId, labels, datasets, chartRef) {
...
function renderXaiShapChart(labels, vals) {
    charts.xaiShap = _makeBarChart('xai-shap-chart', labels, [{
        ...
        borderColor: vals.map(v => v >= 0 ? '#3b82f6' : '#ef4444'),
        borderWidth: 1,
    }], charts.xaiShap);
}
...
function renderXaiLimeChart(labels, vals) {
    charts.xaiLime = _makeBarChart('xai-lime-chart', labels, [{
        ...
        borderColor: vals.map(v => v >= 0 ? '#eab308' : '#ef4444'),
        borderWidth: 1,
    }], charts.xaiLime);
}

function renderXaiGroupedChart(labels, shapVals, limeVals, logitVals) {
    const datasets = [
        { label: 'SHAP', data: shapVals, backgroundColor: 'rgba(59,130,246,0.8)', borderColor: '#3b82f6', borderWidth: 1 },
        { label: 'LIME', data: limeVals, backgroundColor: 'rgba(234,179,8,0.8)',  borderColor: '#eab308', borderWidth: 1 },
    ];
    if (logitVals) {
...
async function runCompareAll() {
    ...
    try {
        ...
        const colorVal = v => {
            const a = Math.abs(v);
            if (a < 0.01) return 'text-zinc-500';
        ...
        const rows = data.results.map(r => {
            const shapCells = fnames.map(f => {
                const v = r.shap_values[f] ?? 0;
                return `<td class="tbl-cell font-mono ${colorVal(v)}">${v.toFixed(3)}</td>`;
            }).join('');
            const limeCells = fnames.map(f => {
                const v = r.lime_weights[f] ?? 0;
                return `<td class="tbl-cell font-mono ${colorVal(v)}">${v.toFixed(3)}</td>`;
            }).join('');
            ...
            <table class="w-full text-left border border-zinc-800 rounded overflow-hidden">
                <thead class="compare-hdr">
                    <tr class="text-[10px] uppercase tracking-wider text-zinc-500 border-b border-zinc-700">
                        <th class="tbl-cell">Model</th>
                        <th class="tbl-cell">Pred.</th>
                        <th class="tbl-cell">P(Pass)</th>
                        ${shapHdrs}${limeHdrs}
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
...
async function openXaiModal() {
    ...
    try {
        ...
        const logitContrib = (lastExplain && selectedModel === 'Logistic Regression')
        ...
        const logitVals = logitContrib ? fnames.map(f => logitContrib[f] ?? 0) : null;
        ...
        dom.xaiValuesGrid.innerHTML = fnames.map((f, i) => `
            <div class="bg-zinc-900/50 border border-zinc-800 rounded p-3 flex flex-col gap-1.5">
                ...
                <div class="text-xs font-mono"><span class="text-blue-400">SHAP: ${shapVals[i].toFixed(4)}</span></div>
                <div class="text-xs font-mono"><span class="text-yellow-400">LIME: ${limeVals[i].toFixed(4)}</span></div>
                ${logitVals ? `<div class="text-xs font-mono"><span class="text-green-400">Logit: ${logitVals[i].toFixed(4)}</span></div>` : ''}
            </div>`
        ).join('');
